...
function setupNewClient(wsClient) {
  ...
  const speechCallback = (stream) => {
    ...
    if (results[0]) {
      ...
      if (isFinal) {
        console.log('Send to client isFinal/text', isFinal, text, streamSid)
        analyze_and_update(text, streamSid)
      }
    }
  };
  ...
  wsClient.on('message', async message => {
    ...
    switch (payload.event) {
      case 'start': {
        ...
        bayeux.getClient().publish('/updates', {event: 'start', streamSid: streamSid});
        return startStream()
      }
      case 'stop': {
        bayeux.getClient().publish('/updates', {event: 'stop', streamSid: streamSid});
        return stopStream()
      }
      case 'media': {
